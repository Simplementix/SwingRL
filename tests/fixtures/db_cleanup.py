...
def drop_migration_artifacts(mgr: DatabaseManager) -> None:
    """Drop all V001-V009 artifacts and clear their schema_migrations ledger rows.
    ...
    *constraint* drop conditional, not the table; calling this when
    ...
    with mgr.connection() as conn:
        ...
        conn.execute(
            ...
            "DELETE FROM schema_migrations WHERE version IN (1, 2, 3, 4, 5, 6, 7, 8, 9); "
            "END IF; "
            "END $$;"
        )
...
def reapply_migrated_schema(mgr: DatabaseManager) -> None:
    ...
    Drops all V001-V009 artifacts (via ``drop_migration_artifacts``) and immediately
