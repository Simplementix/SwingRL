...
1. **Always know which lane you are in** — fast / db / full, budgets above. *(Evidence: the
   only pre-2026-07-22 gate was one monolithic `pytest tests/ -v`, scripts/ci-homelab.sh stage 3 →
...
4. **One spelling for DB gating in new code**: module-level
   ...
   collection-time selection, e.g. tests/data/test_attribution_migration.py::pg_conn.)*
5. **Real DB only when SQL semantics are the subject** (migrations, upserts, FK/view order,
   store layers); mock (`make_mock_db`, tests/conftest.py::make_mock_db) when the DB is a bystander.
...
6. **Never name a real-DB fixture "mock"** — and vice versa. *(Evidence:
   tests/execution/conftest.py::mock_db is a real PostgreSQL DatabaseManager; root
...
7. **Session scope only for immutable values; freeze or don't promote.** DB fixtures stay
   ...
   tests/conftest.py module docstring; only `repo_root` is session-scoped.)*
8. **New autouse fixture ⇒ docstring must justify the per-test tax across ~1,900 tests** and
   ...
   2026-07-22; contrast the well-justified structlog isolator,
   tests/conftest.py::_isolate_structlog_config.)*
9. **Expensive schema fixtures (`db_with_legacy_schema`-class: de-migrate + re-migrate per
   ...
   tests/data/conftest.py::db_with_legacy_schema, ~79 test sites, full V001-V011 drop+reapply
   per test.)*
...
11. **Seeded factories over inline data**: pinned-RNG frames/arrays from conftest factories;
    ...
    tests/conftest.py::equity_ohlcv and the sibling `*_array` factories,
    tests/features/conftest.py — extend, don't fork.)*
12. **One config-YAML factory** — no fourth copy of the ~40-line YAML block. *(Evidence:
    three near-identical blocks: tests/conftest.py::valid_config_yaml and ::equity_env_config_yaml,
    tests/execution/conftest.py::exec_config_yaml.)*
...
14. **Never weaken the DB-safety stack** — suite guard (tests/conftest.py::pytest_configure),
    ...
    (tests/fixtures/db_cleanup.py::ensure_wipe_target_is_test_db), schema preflight
...
- **One xdist session per base URL at a time.** Clone isolation protects the workers
  *within* a session and opt-in serial sessions; two concurrent `pytest -n N` sessions on
  the SAME base URL derive identical clone names (`swingrl_gw0_test` …) and each session's
  `DROP DATABASE … WITH (FORCE)` kills the other's live clone — a loud failure, not
  corruption. Run concurrent sessions against DIFFERENT base URLs.
