...
will be derived at read time under carry item **A1-C13**, so there is nothing for it to hold in either the
...
- **A1-C12 is the only detector for A1-C13's failure mode.** Under read-time adjustment a missing
...
| A missing action is **silent** under read-time adjustment — it mis-scales a symbol's whole pre-event history with no visible bar | Completeness is not a nice-to-have; it is the correctness condition. Detection falls to carry item **A1-C12**'s cross-source check |

Carry item **A1-C13** depends on this dataset: read-time adjustment is only as complete as
