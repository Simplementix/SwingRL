...
| F2: SAC wrote 688k epoch memories (~350 expected); root cause **CONFIRMED by the 2026-07 code review**: SAC's `_on_rollout_end` fires every vec-step (`train_freq=1`, ~167K/fold vs PPO ~82), amplified by old `notable_event` thresholds; no rate-limiting exists today | `epoch_callback.py:325–353,355–377`; SB3 2.7.1 `off_policy_algorithm.py:606` | **§4.10 (D-T3.19)** — caps/triggers + rate-cap task in Plan B |
...
| Risk penalty silently discarded when reward wrapper activates — **CONFIRMED LIVE by the 2026-07 code review**: the wrapper replaces the env reward (sharpe − risk_penalty) with a component sum that has no risk-penalty component, every shaped run trained penalty-free | `reward_wrapper.py:172` vs `envs/base.py:221–238` | **§2.11 (A3)** — fix is a precondition of any L1 harness run (Plan B) |
...
| F1 turbulence column queried but doesn't exist (silent 0.0) — re-confirmed independently by 3 of 4 review agents: the halt has **never fired in production** | `pipeline.py:517,537,541–544`; `postgres_schema.py:224–266` | **Plan A Task 6** (capture-quality blocker per A25) |
| Shadow promotion writes flat `models/active/{env}/`; live trader reads `{env}/{algo}/` — review H2: promotion is a silent no-op + false "promoted" alert | `lifecycle.py:102–186` vs `pipeline.py:364–365`; `promoter.py:123–135` | **Plan A Task D** (was record-only) |
| Live-trader model cache never invalidated (restart required) — review H3: also caches an empty model dict forever if the first cycle precedes deploy | `pipeline.py:354–355,407`; `ensemble.py:121–123` | **Plan A Task D** (was record-only) |
| **C1 (2026-07 review):** portfolio snapshots circular — value copied forward, never marked to market; drawdown/daily-loss breakers and the VIX+drawdown emergency trigger can never trip | `position_tracker.py:47–72,162–196`; `risk_manager.py:123–161`; `emergency.py:456–470` | **Plan A Task A** |
| **C2 (2026-07 review):** Alpaca fills fire-and-forget + equity cron 16:15 ET after close, 7 d/wk — unfilled submits recorded as $0 trades, positions zeroed, next cycle re-buys | `alpaca_adapter.py:124–140`; `pipeline.py:292–295`; `main.py:73–81` | **Plan A Task B** (cycle → 15:45 ET, polling, calendar gate) |
| **H1 (2026-07 review):** feature-health gate wired to an orphan tracker — never trips on real failures; blocks ALL trading after 7 days of uptime | `pipeline.py:81,166` vs `scripts/main.py:257`; `health.py:34,124–135` | **Plan A Task D** |
| **H4 (2026-07 review):** post-halt ramp capacity logged but never applied; full-size trading ~1 day into a 5-day cooldown | `risk_manager.py:182–192`; `circuit_breaker.py:134–139` | **Plan A Task C** |
| **H5 (2026-07 review):** crypto stop-loss detection-only (no order, no alert, no record) and polls the USD book while fills execute on USDT | `stop_polling.py:102–153,128`; `binance_sim.py:232–235` | **Plan A Task C** (minimal); auto-sell deferred, documented |
| Medium/low review findings (M1–M11, L) — full list with dispositions | `docs/superpowers/reviews/2026-07-07-execution-path-code-review.md` | Plan A Tasks A–D folds + Plan B inputs per the review doc's disposition table |
