...
- pattern_presentations_legacy: tracks when patterns are presented to the query agent
  (renamed from pattern_presentations per user ruling 2026-07-17 — the migration V006
  §4.5 table owns the bare name; this legacy writer keeps working until Task 28 cutover)
...
_CREATE_PATTERN_PRESENTATIONS_LEGACY = """
CREATE TABLE IF NOT EXISTS pattern_presentations_legacy (
    id                INTEGER GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    consolidation_id  INTEGER NOT NULL REFERENCES consolidations(id),
...
def init_db() -> None:
    ...
    with get_connection() as conn:
        ...
        _execute_ddl(conn, _CREATE_PATTERN_PRESENTATIONS_LEGACY)
...
def insert_pattern_presentation(
    ...
    with get_connection() as conn:
        cur = conn.execute(
            "INSERT INTO pattern_presentations_legacy "
...
def get_pattern_effectiveness() -> list[dict[str, Any]]:
    """Join pattern_presentations_legacy with pattern_outcomes for human review.
