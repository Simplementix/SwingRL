...
must_haves:
  truths:
    ...
    - "The ci-local skill mirrors stages 2-5 of ci-homelab.sh as 4 native stages (tests, lint, typecheck, security), skipping the Docker build stage"
  artifacts:
    - path: "CLAUDE.md"
  ...
  key_links:
    ...
    - from: ".claude/commands/ci-local.md"
      ...
      via: "mirrors same 4 native quality stages (stages 2-5 of ci-homelab.sh)"
...
"</automated>
  ...
  <done>Five .claude/commands/ files exist (test.md, lint.md, typecheck.md, docker-build.md, ci-local.md); each has YAML frontmatter with description; each contains executable bash commands using uv or docker; ci-local.md contains 4 native stages matching ci-homelab.sh stages 2-5</done>
</task>
...
- ci-local.md has 4 native CI stages matching ci-homelab.sh stages 2-5 (tests, lint, typecheck, security)
